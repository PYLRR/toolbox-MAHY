...
if __name__=="__main__":
    ...
    for dataset in ["MAHY0","MAHY1","MAHY2","MAHY3","MAHY4"]:
        ...
        DETECTIONS_DIR = f"../../../../data/detection/TiSSNet_Pn_raw_repicked/{dataset}"
        ...
        for idx in matched.keys():
            ...
            for si, di in matched[idx][0][2]:
                ...
                cell = np.argmin(np.sqrt((matched[idx][0][3][0] - GRID_TO_COORDS[:, 0]) ** 2 + (
                            111_000 * (matched[idx][0][3][1] - GRID_TO_COORDS[:, 1])) ** 2 + (
                                                     111_000 * (matched[idx][0][3][2] - GRID_TO_COORDS[:, 2])) ** 2))
        ...
        h = dataset[-1]
        cols = ["date", "h_date", "lat", "h_lat", "lon", "h_lon", "depth", "h_depth", "mb",
                                     f"MAHY{h}1", f"MAHY{h}2", f"MAHY{h}3", f"MAHY{h}4"]
        cols = cols[:-1] if dataset == "MAHY2" else cols  # MAHY24 station did not work properly
        S_df_matched.to_csv(f'../../../../data/MAHY/loc_3D/twin-cat/{dataset}_raw.csv', index=False,
                            columns=cols, float_format='%.3f')